...
- Developed using **React Native with TypeScript**, leveraging **Expo** for simplified app building and deployment.  
- Connects to **ESP32 devices** that detect door openings and closings.  
- When an event is detected, the **ESP32** sends a signal to the backend server, built with **Node.js**.  
...
In summary, the **frontend (DoorSentinel)** does not perform complex calculations; it serves as the interface for interacting with the system. The main logic and data management reside in the **backend**.
